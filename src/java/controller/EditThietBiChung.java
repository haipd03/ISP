 ...
 * @author Ngoc Lan
 */
@WebServlet(name = "EditThietBiChung", urlPatterns = {"/editthietbichung"})
public class EditThietBiChung extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        LanDao u = new LanDao();
        if (!g.matches(soPattern) || !sl.matches(soPattern)) {
            request.setAttribute("error", "Dữ liệu nhập vào không hợp lệ!");
            RequestDispatcher dispatcher = request.getRequestDispatcher("listthietbichung");
            dispatcher.forward(request, response);
        } else {
            u.editThietBiChung(tbcid, kid, ten, sl, tt, g);
