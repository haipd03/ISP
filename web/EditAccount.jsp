                            ...
                            <form action="editaccount" method="post">
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-center">
                                     ...
                                     <c:if test="${listA.accountID != 1}">
                                    <div class="flex items-center">
                                        <label class="w-1/3 text-gray-700 font-semibold">Role</label>

                                        <select name="Role" required class="w-2/3 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600">
                                            <option value="1" <c:if test="${listA.role == '1'}">selected</c:if>>Quản Lý</option>
                                            <option value="2" <c:if test="${listA.role == '2'}">selected</c:if>>Vô hiệu hóa</option>
                                            <!--<option value="0" <c:if test="${listA.role == '0'}">selected</c:if>>Chủ</option>-->
                                            </select>
                                    </div>
                                     </c:if>
                                     <c:if test="${listA.accountID == 1}">
                                    <div class="flex items-center">
                                        ...
                                        <select name="Role" readonly required class="w-2/3 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600">
